...
try {
    ...
    if (strlen($code) !== 5) {
        ...
        $sql_compte = "INSERT INTO _compte (idcompte, telephone, motdepasse, email, numadressecompte, ruecompte, ville, codepostal, urlimage) 
        VALUES (:idcompte, :tel, :mdp, :email, :num, :rue, :ville, :code_postal, :urlimage)";
        $stmt_compte = $dbh->prepare($sql_compte);
        $stmt_compte->execute([ ':idcompte' => $idcompte, ':tel' => $tel, ':mdp' => $mdp, ':email' => $email, 
        ':num' => $num,':rue' => $rue, ':ville' => $ville, ':code_postal' => $code, ':urlimage' => 'default.png']);
        ...
        */
        ...
        $sql_pro = "INSERT INTO _comptepro (idcompte, denomination) 
        VALUES (:idcompte, :raison)";
        $sql_pro = $dbh->prepare($sql_pro);
        $sql_pro->execute([
        ':idcompte' => $idcompte, ':raison' => $raison]);
        ...
        if (isset($_FILES['photo'])) {
            ...
            if (move_uploaded_file($_FILES['photo']['tmp_name'], $destination)) {
                ...
                $update_sql = "UPDATE compteprofessionel SET urlimage = :urlimage WHERE idcompte = :idcompte";
        ...
        header('Location: 3_consultation_pro.php');
        exit();
    }
