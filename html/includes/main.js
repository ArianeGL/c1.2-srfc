...
//buttons = document.querySelectorAll(".bouton_toast");
...
function createToast (id) {
	...
	if(id in toastDetails){
		const { typeToast, text } = toastDetails[id];
		const icon = toastType[typeToast]["icon"];
		const toast = document.createElement("li"); // Creating a new 'li' element for the toast
		toast.className = `toast ${id} ${typeToast}`; // Setting the classes for the toast
		...
		console.log("Id du toast invalide");
	}
}
